...
import '../../domain/usecases/get_user_profile.dart'; // Yeni import
import 'user_event.dart';
import 'user_state.dart';

class UserBloc extends Bloc<UserEvent, UserState> {
  final UploadUserPhoto uploadUserPhoto;
  final GetUserProfile getUserProfile; // Yeni bağımlılık
  final LoggerService logger;

  UserBloc({
    required this.uploadUserPhoto,
    required this.getUserProfile, // Yeni bağımlılık
    required this.logger,
  }) : super(UserInitial()) {
    on<UploadUserPhotoEvent>(_onUploadUserPhotoEvent);
    on<LoadUserProfile>(_onLoadUserProfile); // Yeni olay işleyici
  }
